<template>
  <ul>
    <li v-for="(item, index) in getSongList" :key="item.id" class="list">
      <span>{{ index + 1 }}</span>
      <span @click="showSongDetail(item)">{{ item.name }}</span>
      <button @click="addSongToPlayerList_(item)">添加至播放列表</button>
    </li>
  </ul>
</template>
<script>
